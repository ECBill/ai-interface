...
function App() {
  ...
  const [historyDetail, setHistoryDetail] = useState<{ id: number; providerId: string; model: string; status: string; outputText?: string; errorCode?: string; latencyMs?: number } | null>(null)
  ...
  return (
    <main className="shell">
      ...
      {dialog && <div className="modal-backdrop" onClick={() => setDialog(null)}><section className="modal" onClick={(event) => event.stopPropagation()}>
        ...
        {dialog === 'history' && <><span className="label">INVOCATION LOG</span><h2>调用记录</h2>{historyDetail ? <><p className="config-row">{historyDetail.providerId} / {historyDetail.model} · {historyDetail.status} · {historyDetail.latencyMs || '-'}ms</p><div className="history-output">{historyDetail.outputText || historyDetail.errorCode || '未保存响应内容'}</div><button onClick={() => setHistoryDetail(null)}>返回列表</button></> : <>{history.length ? history.map((item) => <button className="history-item" key={item.id} onClick={async () => { setHistoryDetail(await api(`/api/v1/invocations/${item.id}`)) }}>{item.providerId} / {item.model} · {item.status} · {item.latencyMs || '-'}ms</button>) : <p>暂无调用记录</p>}</>}<button onClick={() => { setHistoryDetail(null); setDialog(null) }}>关闭</button></>}
